...
public class PopularBanco {
  ...
  public void iniciar() {
    ...
    if (!usuarios.isEmpty() && pedidoRepository.findAll().isEmpty()) {
      ...
      pedido[0].setUsuario(usuarios.get(0));
      if (usuarios.size() > 1)
        usuarios.remove(0);
      pedidoRepository.save(pedido[0]);
      ...
      pedido[1].setDescricao(
          "Preserva o estilo aventureiro, mas sem abrir mão da descontração? Parabéns, porque fizemos este Copo EVERYDAY Stanley pensando em você! Resistente, durável e atemporal, com preservação térmica de até 4 horas, parede dupla com isolamento a vácuo."
              + System.lineSeparator() +
              "Feito para acompanhar as aventuras da sua legião. Funcional, moderno, versátil, com 3 cores incríveis e uma tampa de acrílico que permite o gole ideal. Versátil para que a sua diversão seja lendária! Seja com vinho, gim, caipirinha, refrigerante ou suco, o Copo Everyday deixa a sua bebida favorita gelada até o último gole."
              + System.lineSeparator() +
      ...
      pedido[1].setUsuario(usuarios.get(0));
      if (usuarios.size() > 1)
        usuarios.remove(0);
      pedidoRepository.save(pedido[1]);
      ...
      pedido[2].setDescricao(
          "Ele é o Devorador de Pecados, o homem que toma para si todos os pecados do mundo, matando todo pecador que vê pela frente - e ele os vê em toda parte! Mas o perigoso lunático pode ter ido longe demais quando tirou a vida de uma grande amiga do Homem-Aranha: a Capitã Jean DeWolff, do Departamento de Polícia de Nova York! Como o enlutado herói aracnídeo vai perseguir um vilão com alvos em todo lugar que vai? Junte-se ao Escalador de Paredes numa busca frenética por Nova York que culminará num momento pivotal entre dois dos maiores heróis mascarados da cidade: Homem-Aranha e Demolidor!");
      ...
      pedido[2].setUsuario(usuarios.get(0));
      if (usuarios.size() > 1)
        usuarios.remove(0);
      pedidoRepository.save(pedido[2]);
    }
  }
